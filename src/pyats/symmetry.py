...
class Symmetry(object):
    ...
    def generate_operators(self):
        ...
        """
        gener, trans, symStr = self.generate_symmetry(self.sym)
    ...
    def generate_symmetry(self, sym):
        ...
        """
        fn = os.path.join(
            os.path.dirname(__file__), "dat_files" + os.sep + "symmetry.dat"
        )
        f = open(fn, "r")
    ...
    def symmetry_order(self, R, T):
        ...
        while (
            np.linalg.norm(RN - np.identity(3)) > tol or np.linalg.norm(TN) > tol
        ) and N < 10:
            RN = np.dot(R, RN)
            TN = (R.dot(TN) + T) % 1
            N += 1
    ...
    def generate_tensor(self, r, q):
        ...
        b = np.exp(
            2j * np.pi * np.einsum("i,ji->j", q, (self.symTr + np.dot(self.symOp, r)))
        )
